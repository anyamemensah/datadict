...
Use the `q_label_width` argument to set how many characters from a question or variable label are displayed for each entry in the dictionary. By default, the dictionary shows up to 100 characters.

``` {r}
generate_dict(data = gss) |>
  dplyr::select(variable_index, question_label) |>
  dplyr::distinct()
...
Finally, `generate_dict()` automatically excludes any variables that contain only `NA` values or only empty strings (`""`) from the dictionary. After running the function, it will print the names of these excluded variables to the console.
