...
With `generate_dict()`, you can use the `max_numeric_display` argument to set how many unique values are shown for each numeric variable. This is particularly helpful when numeric variables have only a few different values, as it lets you display all values rather than summarizing them in a range:
...
generate_dict(data = sample_data, max_numeric_display = 5)
generate_dict(data = sample_data, max_numeric_display = 10)
```
...
`generate_dict()` automatically excludes any variables that contain only `NA` values from the dictionary. After running the function, it will print the names of these excluded variables to the console.
...
generate_dict(data = sample_data2, max_numeric_display = 5)
```
