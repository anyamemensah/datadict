...
.get_q_Label <- function(x, attribute = "label", label_width = label_width) {
...
.return_empty_cols <- function(data) {
  ...
  is_all_empty <- sapply(data, \(col) {sum(as.character(col) == "", na.rm = TRUE) == length(col)})
...
.join_text <- function(text, sep = ", ", before_last = "and ") {
  ...
  if (text_n == 2) return(paste(text, collapse = paste0(" ", before_last)))

  paste0(
    paste(text[1:(text_n - 1)], collapse = sep),
    sep, before_last,
    text[text_n]
  )
